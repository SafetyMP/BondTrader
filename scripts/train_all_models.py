...
from bondtrader.data.training_data_generator import TrainingDataGenerator, save_training_dataset, load_training_dataset
...
class ModelTrainer:
    ...
    def _convert_to_bonds(self, split_data: Dict) -> List[Bond]:
        ...
        for i, (features, target, metadata) in enumerate(zip(
            split_data['features'],
            split_data['targets'],
            split_data['metadata']
        )):
    ...
    def train_all_models(self, resume: bool = False) -> Dict:
        ...
        results['ml_adjuster'] = train_with_timing(
            1, 'ml_adjuster', train_ml_adjuster, 
            validation_func=validate_ml_model
        )
        ...
        try:
            ...
            opt_results = bayesian_opt.optimize_hyperparameters(
                sample_bonds,
                param_bounds={'n_estimators': (50, 200)},
                num_iterations=20
            )
                    ...
                    try:
                        ...
                        for bond in self.test_bonds[:100]:  # Sample for speed
                            try:
                                if hasattr(model, 'predict_adjusted_value'):
                                    pred = model.predict_adjusted_value(bond)
                                    test_predictions.append(pred.get('ml_adjusted_value', pred.get('ml_adjusted_fair_value', bond.current_price)))
                ...
                for benchmark_name in ['bloomberg', 'aladdin', 'goldman', 'jpmorgan']:
                    try:
                        individual_drift = self.drift_detector.calculate_drift(
                            evaluation_bonds,
                            predictions,
                            benchmark_methodology=benchmark_name
                        )
...
def main():
    ...
    trainer = ModelTrainer(
        ...
        generate_new=False  # Set to True to generate new dataset
    )
    ...
    if 'drift_analysis' in results:
        ...
        for model_name, drift_data in results['drift_analysis'].items():
            if 'consensus_drift' in drift_data:
                ...
                print(f"{model_name:<30} {drift.drift_score:<15.4f} {drift.root_mean_squared_error:<15.2f} "
