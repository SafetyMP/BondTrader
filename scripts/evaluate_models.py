    ...
    for model_name, model_results in evaluation_results.items():
        ...
        if 'normal_market' in model_results:
            ...
            if metrics.drift_vs_consensus.correlation < 0.85:
                report['warnings'].append(f"{model_name}: Low benchmark correlation ({metrics.drift_vs_consensus.correlation:.4f})")
...
def print_evaluation_summary(evaluation_report: Dict):
    ...
    if 'best_performing_models' in evaluation_report:
        ...
        if 'highest_correlation' in best:
            print(f"  Highest Correlation: {best['highest_correlation']['model']} ({best['highest_correlation']['correlation']:.4f})")
...
if __name__ == "__main__":
    ...
    results = evaluate_all_models(
        generate_new_evaluation=False,  # Set to True to regenerate evaluation dataset
        save_results=True
    )
