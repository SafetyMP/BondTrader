...
def main():
    ...
    with tab2:
        ...
        if opportunities:
            ...
            st.dataframe(
                opp_df[[
                    'bond_id', 'bond_type', 'issuer', 'recommendation',
                    'market_price', 'adjusted_fair_value', 'profit_percentage',
                    'ytm', 'credit_rating', 'arbitrage_type'
                ]].rename(columns={
                    'bond_id': 'Bond ID',
                    'bond_type': 'Type',
                    'issuer': 'Issuer',
    ...
    with tab3:
        ...
        if comparisons:
            ...
            st.dataframe(
                display_df[[
                    'bond_id', 'group', 'bond_type', 'market_price',
                    'fair_value', 'relative_mispricing_pct'
                ]].rename(columns={
                    'bond_id': 'Bond ID',
                    'group': 'Group',
                    'bond_type': 'Type',
    ...
    with tab4:
        ...
        fig_mismatch.update_layout(
            title=f"Value Comparison for {selected_bond.bond_id}",
            yaxis_title="Value ($)"
        )
    ...
    with tab8:
        ...
        if risk_subtab == "Credit Risk":
            ...
            if not liquidity_df.empty:
                ...
                st.dataframe(
                    liquidity_df[['bond_id', 'rating', 'spread_bps', 'depth_ratio', 
                                 'liquidity_rating', 'total_cost']].rename(columns={
                        'bond_id': 'Bond ID',
                        'rating': 'Rating',
            ...
            if multi_curve.ois_curve and multi_curve.libor_curve:
                curve_df = pd.DataFrame({
                    ...
                    'Basis Spread': [multi_curve.calculate_basis_spread(m)*10000 
    ...
    with tab9:
        ...
        if opt_method == "Markowitz":
            ...
            target_return = st.number_input("Target Return (%)", 0.0, 20.0, None, step=0.1) / 100 if st.checkbox("Set Target Return", key='target_ret') else None
    ...
    with tab10:
        ...
        fig_factor_risk = px.bar(factor_risk_df, x='Factor', y='Risk Contribution %', 
    ...
    with tab11:
        ...
        if exec_subtab == "Backtesting":
            ...
            if st.button("Run Backtest"):
                ...
                fig_perf = px.line(perf_df, x='Period', y='Portfolio Value', 
            ...
            if strategy_type == "TWAP":
                ...
                st.dataframe(schedule_df[['interval', 'time', 'quantity', 'cumulative_quantity']], 
                           use_container_width=True)
            ...
            if floating_bonds:
                ...
                next_reset = st.date_input("Next Reset Date", 
                                          value=datetime.now().date() + timedelta(days=182))
    ...
    with tab12:
        ...
        if adv_ml_subtab == "Ensemble ML":
                ...
                with st.spinner("Training ensemble (this may take a minute)..."):
                    try:
                        ...
                        model_df = pd.DataFrame([
                            {'Model': name, 'Test R²': f"{metrics['test_r2']:.4f}", 
                        ...
                        st.metric("Ensemble Improvement", 
                        ...
                        if 'sorted' in importance:
                            ...
                            fig_importance = px.bar(feature_df, x='Feature', y='Importance',
            ...
            if advanced_ml.is_trained:
                ...
                with col2:
                    st.metric("Lower Bound (95% CI)", format_currency(uncertainty_result['confidence_interval_lower']))
                with col3:
                    st.metric("Upper Bound (95% CI)", format_currency(uncertainty_result['confidence_interval_upper']))
            ...
            if st.button("Run AutoML", key='run_automl'):
                with st.spinner("Running AutoML (evaluating multiple models)..."):
                    try:
                        ...
                        fig_comparison = px.bar(comparison_df, x='Model', y='R² Score',
            ...
            data_type = st.selectbox("Data Type", ["ESG Analysis", "Sentiment Analysis", "Economic Factors"], key='alt_data_type')
            ...
            if data_type == "ESG Analysis":
                ...
                fig_esg.add_trace(go.Bar(
                    ...
                    y=[esg_result['environmental_score'], esg_result['social_score'], 
                ...
                with col3:
                    st.metric("Sentiment-Adjusted Value", format_currency(sentiment_result['sentiment_adjusted_value']))
