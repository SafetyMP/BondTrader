...
class ModelEvaluator:
    ...
    def generate_or_load_evaluation_dataset(
        ...
        num_bonds: int = 1000  # Reduced for faster execution
    ) -> Dict:
    ...
    def evaluate_all_models(
        ...
        if use_parallel and len(models) > 1:
            ...
            model_iter = tqdm(model_items, desc="Evaluating models", 
    ...
    def _evaluate_models_parallel(
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            completed = tqdm(as_completed(futures), total=len(futures), 
                           desc="Parallel evaluation", disable=not TQDM_AVAILABLE)
            ...
            for future in completed:
                ...
                if result.get('status') == 'success' and TQDM_AVAILABLE:
                    ...
                    completed.set_postfix({
                        f'{model_name}': f"{score:.1f} ({eval_time:.1f}s)"
                    })
...
def main():
    ...
    evaluation_dataset = evaluator.generate_or_load_evaluation_dataset(
        generate_new=True,  # Generate new dataset
        num_bonds=1000  # Reduced for faster execution
    )
