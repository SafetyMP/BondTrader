...
class EnhancedMLBondAdjuster:
    ...
    def _create_enhanced_features(self, bonds: List[Bond], fair_values: List[float]) -> Tuple[np.ndarray, List[str]]:
        ...
        feature_names = [
            ...
            'callable', 'convertible', 'ytm', 'duration', 'convexity',
            'price_to_fair_ratio', 'face_value', 'modified_duration',
            'spread_over_rf', 'time_decay', 'quarter', 'day_of_year'
        ]
        ...
        for bond, fv in zip(bonds, fair_values):
            ...
            time_decay = char['time_to_maturity'] / (char['years_since_issue'] + char['time_to_maturity']) if (char['years_since_issue'] + char['time_to_maturity']) > 0 else 0
