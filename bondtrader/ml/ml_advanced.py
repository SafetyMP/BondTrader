...
class AdvancedMLBondAdjuster:
    ...
    def _create_advanced_features(self, bonds: List[Bond], fair_values: List[float]) -> Tuple[np.ndarray, List[str]]:
        ...
        # Feature names
        base_names = [
            'coupon_rate', 'time_to_maturity', 'credit_rating', 'price_to_par',
            'years_since_issue', 'frequency', 'callable', 'convertible',
            'ytm', 'duration', 'convexity', 'price_to_fair_ratio', 'face_value',
            'modified_duration', 'spread_over_rf'
        ]
        poly_names = [
            'coupon_rate^2', 'ttm^2', 'duration^2',
            'coupon*ttm', 'coupon*duration', 'ttm*duration'
        ]
        inter_names = [
            'price_to_par*duration', 'rating*ttm',
            'ytm*duration', 'convexity*duration'
        ]
    ...
    def train_ensemble(
        ...
        y = np.array([bond.current_price / fv if fv > 0 else 1.0 
                     for bond, fv in zip(bonds, fair_values)])
        ...
        gb_model = GradientBoostingRegressor(
            n_estimators=300, max_depth=7, learning_rate=0.05, min_samples_split=5,
            min_samples_leaf=2, subsample=0.9, random_state=random_state
        )
        nn_model = MLPRegressor(
            hidden_layer_sizes=(150, 100), max_iter=1000, random_state=random_state,
            alpha=0.01, learning_rate='adaptive', early_stopping=True, validation_fraction=0.1
        )
    ...
    def get_feature_importance_explained(self) -> Dict:
        ...
        if 'consensus' in importance_results:
            sorted_importance = sorted(
                importance_results['consensus'].items(),
                key=lambda x: x[1],
                reverse=True
            )
