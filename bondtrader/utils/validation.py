...
def validate_bond_input(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        if args and hasattr(args[0], '__class__'):
            ...
            bond = args[1] if len(args) > 1 and args[0].__class__.__name__ in ['BondValuator', 'ArbitrageDetector'] else (args[0] if len(args) > 0 else None)
...
def validate_weights_sum(weights: List[float], expected_sum: float = 1.0, tolerance: float = 1e-6, 
                         name: str = "weights") -> None:
    """
