...
class CreditRiskEnhanced:
    ...
    def _get_recovery_rate(self, rating: str) -> float:
        ...
        recovery_map = {
            'AAA': 0.60, 'AA': 0.58, 'AA+': 0.58, 'AA-': 0.56,
            'A+': 0.56, 'A': 0.54, 'A-': 0.52,
            'BBB+': 0.50, 'BBB': 0.48, 'BBB-': 0.46,
            'BB+': 0.44, 'BB': 0.42, 'BB-': 0.40,
            'B+': 0.38, 'B': 0.36, 'B-': 0.34,
            'CCC+': 0.32, 'CCC': 0.30, 'CCC-': 0.28,
            'D': 0.20, 'NR': 0.40
        }
    ...
    def credit_migration_analysis(
        ...
        if current_rating not in self.migration_matrix:
            ...
            migration_probs = {
                'AAA': 0.01, 'AA': 0.05, 'A': 0.10, 'BBB': 0.40,
                'BB': 0.30, 'B': 0.10, 'CCC': 0.03, 'D': 0.01
            }
        else:
    ...
    def calculate_credit_var(
        ...
        current_portfolio_value = sum(b.current_price * w * b.face_value 
