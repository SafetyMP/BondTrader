...
class LiquidityRiskEnhanced:
    ...
    def _estimate_spread_from_characteristics(self, bond: Bond) -> float:
        ...
        rating_spreads = {
            'AAA': 2, 'AA+': 3, 'AA': 4, 'AA-': 5,
            'A+': 6, 'A': 8, 'A-': 10,
            'BBB+': 15, 'BBB': 20, 'BBB-': 25,
            'BB+': 35, 'BB': 50, 'BB-': 75,
            'B+': 100, 'B': 150, 'B-': 200,
            'CCC+': 300, 'CCC': 400, 'CCC-': 500,
            'D': 1000, 'NR': 25
        }
    ...
    def estimate_market_depth(
        self,
        bond: Bond,
        trade_size: float = 100000  # Trade size in face value
    ) -> Dict:
        """
    ...
    def calculate_lvar(
        ...
        var_result = risk_manager.calculate_var(
            bonds, weights, confidence_level,
            time_horizon=int(liquidation_horizon),
            method='monte_carlo'
        )
        
        var_value = var_result['var_value']
        portfolio_value = sum(b.current_price * w * b.face_value 
    ...
    def analyze_liquidity_risk(
        ...
        return {
            ...
            'liquidity_cost_pct': ((total_spread_cost + total_impact_cost) / portfolio_value) * 100 if portfolio_value > 0 else 0,
