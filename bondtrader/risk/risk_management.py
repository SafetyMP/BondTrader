...
class RiskManager:
    ...
    def _get_default_probability(self, rating: str) -> float:
        ...
        default_map = {
            ...
            'A+': 0.003, 'A': 0.005, 'A-': 0.008,
            'BBB+': 0.012, 'BBB': 0.020, 'BBB-': 0.035,
            'BB+': 0.050, 'BB': 0.080, 'BB-': 0.120,
            'B+': 0.180, 'B': 0.250, 'B-': 0.350,
            'CCC+': 0.450, 'CCC': 0.550, 'CCC-': 0.650,
            'D': 1.000, 'NR': 0.020
        }
    ...
    def _get_recovery_rate(self, rating: str) -> float:
        ...
        rating_to_recovery = {
            'AAA': 0.60, 'AA+': 0.58, 'AA': 0.56, 'AA-': 0.54,
            'A+': 0.52, 'A': 0.50, 'A-': 0.48,
            'BBB+': 0.46, 'BBB': 0.44, 'BBB-': 0.42,
            'BB+': 0.40, 'BB': 0.38, 'BB-': 0.36,
            'B+': 0.34, 'B': 0.32, 'B-': 0.30,
            'CCC+': 0.28, 'CCC': 0.26, 'CCC-': 0.24,
            'D': 0.20, 'NR': 0.40
        }
    ...
    def calculate_interest_rate_sensitivity(
        self,
        bond: Bond,
        rate_change: float = 0.01  # 1% change
    ) -> Dict:
        """
    ...
    def stress_test(
        self,
        bonds: List[Bond],
        scenario: str = 'rate_shock',
        shock_size: float = 0.02
    ) -> Dict:
        """
