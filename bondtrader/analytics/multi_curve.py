...
class MultiCurveFramework:
    ...
    def build_ois_curve(
        ...
        if interpolation_method == 'linear':
            ...
            interp_func = interp1d(maturities, rates, kind='cubic',
                                 fill_value='extrapolate', bounds_error=False)
        elif interpolation_method == 'log_linear':
            # Log-linear interpolation on rates
            log_rates = np.log(1 + rates)
            interp_func_log = interp1d(maturities, log_rates, kind='linear',
                                      fill_value='extrapolate', bounds_error=False)
            def interp_func(t):
    ...
    def price_bond_with_multi_curve(
        self,
        bond: Bond,
        use_ois_discounting: bool = True
    ) -> Dict:
        """
