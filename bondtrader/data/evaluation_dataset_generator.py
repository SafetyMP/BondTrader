...
class EvaluationDatasetGenerator:
    ...
    def _generate_evaluation_bond_universe(self, num_bonds: int) -> List[Bond]:
        ...
        bond_types = [BondType.TREASURY, BondType.CORPORATE, BondType.HIGH_YIELD,
                     BondType.FIXED_RATE, BondType.ZERO_COUPON]
        
        # Maturity distribution
        maturity_ranges = [(0.5, 2), (2, 5), (5, 10), (10, 20), (20, 30)]
        
        issuers = [
            "US Treasury", "Apple Inc", "Microsoft Corp", "JPMorgan Chase",
            "Bank of America", "Goldman Sachs", "Exxon Mobil", "AT&T Inc",
            "Verizon Communications", "Coca-Cola Co", "Walmart Inc",
            "Amazon.com Inc", "Google LLC", "Meta Platforms", "Tesla Inc",
            "General Electric", "Ford Motor Co", "General Motors",
            "Boeing Co", "Lockheed Martin", "Raytheon Technologies"
        ]
    ...
    def _generate_scenario_data(
        ...
        for bond in bonds:
            ...
            fair_value = self.valuator.calculate_fair_value(
                bond, required_yield=required_ytm
            )
            ...
            market_price = fair_value * (
                1 + liquidity_noise + sentiment_impact +
    ...
    def evaluate_model(
        ...
        scenario_iter = tqdm(scenarios_to_eval.items(), desc="Evaluating scenarios", 
                            total=total_scenarios, disable=not TQDM_AVAILABLE)
    ...
    def _predict_parallel(self, model, bonds: List[Bond], max_workers: Optional[int], batch_size: int) -> List[float]:
        ...
        def predict_single(bond):
            try:
                if hasattr(model, 'predict_adjusted_value'):
                    ...
                    return pred.get('ml_adjusted_value', 
                                   pred.get('ml_adjusted_fair_value', 
                                          bond.current_price))
        ...
        for batch in tqdm(bond_batches, desc="Processing batches", 
    ...
    def _predict_sequential(self, model, bonds: List[Bond], batch_size: int) -> List[float]:
        ...
        for batch in tqdm(bond_batches, desc="Processing bonds", 
                         disable=not TQDM_AVAILABLE or len(bond_batches) < 5):
            for bond in batch:
                try:
                    if hasattr(model, 'predict_adjusted_value'):
                        pred = model.predict_adjusted_value(bond)
                        value = pred.get('ml_adjusted_value', 
                                       pred.get('ml_adjusted_fair_value', 
                                              bond.current_price))
                    elif hasattr(model, 'predict'):
                        char = bond.get_bond_characteristics()
    ...
    def _calculate_evaluation_metrics(
        ...
        if np.any(non_zero_mask):
            mape = np.mean(np.abs((actual_prices[non_zero_mask] - predictions[non_zero_mask]) / 
                                 actual_prices[non_zero_mask])) * 100
        else:
            mape = float('inf')
    ...
    def _generate_evaluation_metadata(
        ...
        metadata = {
            ...
            'total_bonds': sum(
                len(sc['bonds']) for sc in evaluation_results.values() 
            ...
            for sc_name, sc_data in value.items():
                if sc_name == 'benchmarks':
                    ...
                    serializable_scenarios[sc_name] = {
                        ...
                        'actual_prices': sc_data['actual_prices'].tolist() if isinstance(sc_data['actual_prices'], np.ndarray) else sc_data['actual_prices'],
                        'fair_values': sc_data['fair_values'].tolist() if isinstance(sc_data['fair_values'], np.ndarray) else sc_data['fair_values'],
